  ...
  methods: {
    addFormSub(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          let url = 'http://yhocn.cn:8102/peizhi/addChaChong';
          axios.post(url, this.addForm).then(res => {
            if (res.data.data.length == 0) {

              let url = "http://yhocn.cn:8102/peizhi/peizhiAdd";
    ...
    updFormSub(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          let url = 'http://yhocn.cn:8102/peizhi/updChaChong';
          axios.post(url, this.addForm).then(res => {
            if (res.data.data.length == 0) {

              let url = "http://yhocn.cn:8102/peizhi/peizhiUpd";
              this.axios.post(url, this.addForm).then(res => {
                if (res.data.code == '00') {
                  MessageUtil.success("添加成功");
                  this.queryByType();
    ...
    delConfig() {
      ...
      this.$confirm('是否删除选中信息?', '提示', {
        ...
        let url = "http://yhocn.cn:8102/peizhi/delPeizhi";
    ...
    moRenConfig() {
      ...
      this.$confirm('是否将当前信息设置为默认?', '提示', {
        ...
        let url = "http://yhocn.cn:8102/user/updateMoRen";
        axios.post(url,
            {"type": this.PAGE_TYPE,
