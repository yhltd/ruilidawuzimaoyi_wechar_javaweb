    ...
    <el-dialog title="" :visible.sync="addDialog" width="90%">
      ...
      <el-form :model="addForm" ref="addUsr" label-width="100px"
        ...
        <el-row :gutter="15">
          ...
          <el-col :span="8">
            <el-form-item label="供应商名称" prop="name" class="custom-form-item">
              <el-select style="z-index:3000;" v-model="addForm.name" @change="changeKeHu()" clearable filterable placeholder="请选择供应商">
                <!-- types 为后端查询 -->
                <el-option
                    style="z-index:3000;"
...
export default {
  ...
  methods:{
    ...
    getAll(){
      ...
      let url = "http://yhocn.cn:8102/qiChuGongYingShang/getAll"
    ...
    saveShangPinGuiGe(){
      let url = "http://yhocn.cn:8102/qiChuGongYingShang/add";
    ...
    updShangPinGuiGe(){
      let url = "http://yhocn.cn:8102/qiChuGongYingShang/upd";
      this.axios.post(url, this.addForm).then(res => {
        if (res.data.code == '00') {
          MessageUtil.success("添加成功");
          this.addDialog = false;
    ...
    deleteClick(){
      ...
      this.$confirm('是否删除当前选中的信息?', '提示', {
        ...
        console.log(list)
        let url = "http://yhocn.cn:8102/qiChuGongYingShang/del";
