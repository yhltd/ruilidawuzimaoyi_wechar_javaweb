...
export default {
  ...
  created() {
    this.axios
      .get("http://localhost:8081/account/getall")
      .then((res) => {
  ...
  methods: {
    ...
    SubmitEvent() {
      //确定事件
      this.axios
        .post("http://localhost:8081/account/add" + "/" + this.ruleForm.aname)
        .then((res) => {
          if (res.data == "success") {
            this.$message({
              message: "恭喜你，添加成功",
    ...
    edit() {
      this.axios
        .put(
          "http://localhost:8081/account/update" +
            "/" +
    ...
    del(data) {
      ...
      this.$confirm('此操作将删除该数据, 是否继续?', '提示', {
          ...
          this.axios
            .delete("http://localhost:8081/account/delete" + "/" + data.aid)
            .then((res) => {
              if (res.data == "success") {
                this.$message({
                  message: "删除成功",
