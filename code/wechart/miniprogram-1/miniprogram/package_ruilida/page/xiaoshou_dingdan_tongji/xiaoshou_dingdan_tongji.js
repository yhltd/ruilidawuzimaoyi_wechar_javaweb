...
Page({
  ...
  tableShow: function () {
    ...
    if(type == '按业务员'){
      title[0].name = "业务员"
      title[0].text = "业务员"
      sql = "select yewuyuan as name,count(yewuyuan) as num,sum(convert(float,isnull(jiashui_heji,0))) as money from xiaoshou_dingdan where riqi >= '" + start_date + "' and riqi <= '" + stop_date + "' group by yewuyuan order by money desc,num desc"
    }else if(type == '按客户'){
      title[0].name = "客户"
      title[0].text = "客户"
      sql = "select kehu as name,count(yewuyuan) as num,sum(convert(float,isnull(jiashui_heji,0))) as money from xiaoshou_dingdan where riqi >= '" + start_date + "' and riqi <= '" + stop_date + "' group by kehu order by money desc,num desc"
    }else if(type == '按产品'){
      title[0].name = "产品名称"
      title[0].text = "产品名称"
      sql = "select shangpin_mingcheng as name,sum(convert(float,isnull(shuliang,0))) as num,sum(convert(float,isnull(jiashui_xiaoji,0))) as money from xiaoshou_dingdan as dingdan left join xiaoshou_dingdan_item as item on dingdan.id = item.xiaoshou_id where riqi >= '" + start_date + "' and riqi <= '" + stop_date + "' group by shangpin_mingcheng order by money desc,num desc"
    }
    console.log(sql)
    wx.cloud.callFunction({
      ...
      success: res => {
        ...
        for(var i=0; i<list.length; i++){
          list[i].num = Math.round(list[i].num * 100) / 100
