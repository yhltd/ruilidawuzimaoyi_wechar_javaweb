...
Page({
  ...
  sel1:function(){
    ...
    var e = [start_date,stop_date,_this.data.zhuanru,_this.data.zhuanchu]
    _this.qxShow()
    _this.tableShow(e)
  },
  ...
  tableShow: function (e) {
    ...
    if(userInfo.power_mingxi.zhuanzhang_sel != '是'){
      wx.showToast({
        title: '当前账号无权限',
        icon: 'none'
      })
    ...
    var sql = "select * from zhuanzhang where riqi >= '" + e[0] + "' and riqi <= '" + e[1] + "' and zhuanru like '%" + e[2] + "%' and zhuanchu like '%" + e[3] + "%';select * from peizhi where type ='收款账户'"
    console.log(sql)
