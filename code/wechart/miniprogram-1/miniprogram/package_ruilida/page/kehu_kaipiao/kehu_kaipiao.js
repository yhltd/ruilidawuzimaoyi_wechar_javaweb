...
Page({
  ...
  tableShow: function () {
    ...
    if(type == '按核算单位'){
      title[0].name = "核算单位"
      title[0].text = "核算单位"
      sql = "select kaipiao_danwei as name,sum(convert(float,isnull(jiashui_heji,0))) as jiashui_heji from xiaoshou_kaipiao where kaipiao_riqi >= '" + start_date + "' and kaipiao_riqi <= '" + stop_date + "' group by kaipiao_danwei"
    }else if(type == '按客户'){
      title[0].name = "客户"
      title[0].text = "客户"
      sql = "select shoupiao_danwei as name,sum(convert(float,isnull(jiashui_heji,0))) as jiashui_heji from xiaoshou_kaipiao where kaipiao_riqi >= '" + start_date + "' and kaipiao_riqi <= '" + stop_date + "' group by shoupiao_danwei"
    }
    console.log(sql)
    wx.cloud.callFunction({
      ...
      success: res => {
        ...
        for(var i=0; i<list.length; i++){
          list[i].jiashui_heji = Math.round(list[i].jiashui_heji * 100) / 100
        }
        _this.setData({
          list,
          title
