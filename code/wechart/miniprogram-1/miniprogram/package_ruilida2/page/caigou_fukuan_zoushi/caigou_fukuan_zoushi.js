...
Page({
  ...
  tableShow: function (e) {
    ...
    if(e[0] == '按日'){
      sql = "select shouzhi_riqi as riqi,jizhangren as yewuyuan,sum(convert(float,isnull(jizhang_jine,0)) + convert(float,isnull(kedi_shuie,0))) as money from shouzhi_mingxi where (danju_leixing = '采购订单' or danju_leixing = '采购入库') and shouzhi_riqi >='" + e[1] + "' and shouzhi_riqi <='" + e[1] + "-31' group by shouzhi_riqi,jizhangren;select jizhangren as yewuyuan from shouzhi_mingxi where (danju_leixing = '采购订单' or danju_leixing = '采购入库') and shouzhi_riqi >='" + e[1] + "' and shouzhi_riqi <='" + e[1] + "-31' group by jizhangren"
    }else if(e[0] == '按月'){
      sql = "select SUBSTRING(shouzhi_riqi,1,7) as riqi,jizhangren as yewuyuan,sum(convert(float,isnull(jizhang_jine,0)) + convert(float,isnull(kedi_shuie,0))) as money from shouzhi_mingxi where (danju_leixing = '采购订单' or danju_leixing = '采购入库') and SUBSTRING(shouzhi_riqi,1,7) >= '" + e[1] + "-01' and SUBSTRING(shouzhi_riqi,1,7) <= '" + e[1] + "-12' group by shouzhi_riqi,jizhangren;select jizhangren as yewuyuan from shouzhi_mingxi where (danju_leixing = '采购订单' or danju_leixing = '采购入库') and SUBSTRING(shouzhi_riqi,1,7) >= '" + e[1] + "-01' and SUBSTRING(shouzhi_riqi,1,7) <= '" + e[1] + "-12' group by jizhangren"
    }
    console.log(sql)
