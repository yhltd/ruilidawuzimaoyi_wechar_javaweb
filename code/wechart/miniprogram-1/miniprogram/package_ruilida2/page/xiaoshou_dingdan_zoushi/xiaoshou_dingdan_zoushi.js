...
Page({
  ...
  tableShow: function (e) {
    ...
    if(e[0] == '按日'){
      sql = "select riqi,yewuyuan,sum(convert(float,isnull(jiashui_heji,0))) as money from xiaoshou_dingdan where riqi >='" + e[1] + "' and riqi <='" + e[1] + "-31' group by riqi,yewuyuan;select yewuyuan from xiaoshou_dingdan  where riqi >='" + e[1] + "' and riqi <='" + e[1] + "-31' group by yewuyuan"
      // sql = "select shouzhi_riqi,jizhang_type,sum(money) as money from (select shouzhi_riqi,jizhang_type,round(convert(float,isnull(jizhang_jine,0)) + convert(float,isnull(kedi_shuie,0)),2) as money from shouzhi_mingxi where shouzhi_type = '收入') as shouzhi where shouzhi_riqi >= '" + e[1] + "-01' and shouzhi_riqi <= '" + e[1] + "-31' group by shouzhi_riqi,jizhang_type;select jizhang_type from shouzhi_mingxi where shouzhi_riqi >= '" + e[1] + "-01' and shouzhi_riqi <= '" + e[1] + "-31' and shouzhi_type = '收入' group by jizhang_type"
    }else if(e[0] == '按月'){
      sql = "select SUBSTRING(riqi,1,7) as riqi,yewuyuan,sum(convert(float,isnull(jiashui_heji,0))) as money from xiaoshou_dingdan where SUBSTRING(riqi,1,7) >= '" + e[1] + "-01' and SUBSTRING(riqi,1,7) <= '" + e[1] + "-12' group by SUBSTRING(riqi,1,7),yewuyuan;select yewuyuan from xiaoshou_dingdan where SUBSTRING(riqi,1,7) >= '" + e[1] + "-01' and SUBSTRING(riqi,1,7) <= '" + e[1] + "-12' group by yewuyuan"
      // sql = "select shouzhi_riqi,jizhang_type,sum(money) as money from (select SUBSTRING(shouzhi_riqi,1,7) as shouzhi_riqi,jizhang_type,round(convert(float,isnull(jizhang_jine,0)) + convert(float,isnull(kedi_shuie,0)),2) as money from shouzhi_mingxi where shouzhi_type = '收入') as shouzhi where shouzhi_riqi >= '" + e[1] + "-01' and shouzhi_riqi <= '" + e[1] + "-12' group by shouzhi_riqi,jizhang_type;select jizhang_type from shouzhi_mingxi where SUBSTRING(shouzhi_riqi,1,7) >= '" + e[1] + "-01' and SUBSTRING(shouzhi_riqi,1,7) <= '" + e[1] + "-12' and shouzhi_type = '收入' group by jizhang_type"
    }
    console.log(sql)
          ...
          for(var i=0; i<type_list.length; i++){
            for(var j=money_list.length-1; j>=0; j--){
              if(type_list[i].yewuyuan == money_list[j].yewuyuan){
                ...
                y_list[this_column] = y_list[this_column] + (money_list[j].money * 1)
          ...
          for(var i=0; i<type_list.length; i++){
            for(var j=money_list.length-1; j>=0; j--){
              if(type_list[i].yewuyuan == money_list[j].yewuyuan){
                ...
                this_column = this_column + ""
                type_list[i][this_column] = money_list[j].money
                y_list[this_column-1] = y_list[this_column-1] + (money_list[j].money * 1)
                if(type_list[i].sum == undefined){
                  type_list[i].sum = money_list[j].money
                }else{
                  type_list[i].sum = (type_list[i].sum * 1) + (money_list[j].money * 1)
